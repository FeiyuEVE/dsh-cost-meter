...
[![version](https://img.shields.io/badge/version-1.6.12-4176E6)](https://github.com/Han-1413141/dsh-cost-meter)
...
| Call details (SQLite) | Recorded automatically in the background | Every model call is written to a SQLite call-details database ($DSH_HOME/storages/cost-meter/calls.sqlite): workspace (session cwd) / session id / session title / time / input & output tokens / cache hits / cost, with all costs normalized to RMB; today/month/cumulative cards, history and cross-day session rankings all read from this database, and pre-upgrade ledger history is migrated automatically (idempotent); vendor price tables support per-vendor and per-model USD/CNY currency configuration |
...
| Auto model routing | Settings (Cost) / below the input | Automatically switches the active session's provider to DeepSeek official during off-peak hours and to OpenCode Go during peak hours (both routes default to `deepseek-v4-flash` with max reasoning, configurable per side); applies to the session you are viewing (effective from its next request), and only while its current model is flash — after you pick another model manually it stays out; the dock chip shows the current tier/route (click to turn off); check interval configurable 1-60 minutes |
...
```sh
dsh plugin --profile web add github:Han-1413141/dsh-cost-meter#v1.6.12
```

Without git, use the GitHub tag archive:

```sh
dsh plugin --profile web add https://github.com/Han-1413141/dsh-cost-meter/archive/refs/tags/v1.6.12.tar.gz
```
