...
## v1.7.1(2026-08-30,本地改造)—— 🔀 自动模型路由:平价用 DeepSeek 官方,高峰用 OpenCode Go
- **新增:峰谷时段自动切换正在运行的会话的路由**——平价(谷)时段自动把当前会话的 provider 切到 DeepSeek 官方,高峰时段自动切到 OpenCode Go;两侧模型默认 `deepseek-v4-flash` + `max` 思考深度,逐侧可配。档位规则与峰谷计价一致(UTC 01:00–04:00、06:00–10:00 为峰;周末全天平价)。
- **作用到当前正在查看的会话,下一轮请求生效**:切换经宿主会话控制服务写入会话级模型选择(与界面模型选择器同一通道),无需手动操作。
- **只对 flash 会话生效**:当前会话模型不是 `deepseek-v4-flash`(如手动改选了其它模型)时自动路由不干预;切换只改 provider、模型保持 flash。
- **开关与状态**:设置页「费用设置」新增「自动模型路由」面板(开关、检查间隔 1-60 分钟、双侧路由编辑);输入框下方显示当前档位/路由 chip(点击一键关闭),并展示上次切换时刻、跳过原因(等待会话/非 flash/服务不可用)与失败提示。
- **默认关闭**,开启后按档位轮询(默认 1 分钟),档位变化才执行切换。
...
- **新增:已命中模型可改映射 + 本地模型零消耗开关**——设置页「模型名匹配」的目标下拉新增「本地模型（零消耗）」:任意模型(含已自动命中的云端模型)都可手动标记为本地,写入后历史金额即时归零;新增「本月已命中价格的模型」区块,已命中的模型同样允许改挂其它价格条目。
- **改进:今日费用实时刷新**——侧边栏「今日费用」此前靠 60 秒轮询,对话结束后平均要等半分钟才变;现在监听会话用量的实时推送,流结束约 1 秒内更新,60 秒轮询保留兜底。同时余额/额度快照改为「有旧值立即返回、后台静默刷新」,刷新不再被网络查询卡住。

## v1.6.10(2026-08-29)—— 🌏 跨天会话「今日 ¥0」:宿主/浏览器时区错位提示(issue #74)
- **排查结论:跨天会话的费用本就按「调用发生日」归账**——每次调用按发起时刻记入当日(与官方请求侧计费一致),同一会话跨天会在两个日条目下各有会话行;新增行为级测试锁定该保证。
- **根因:宿主进程时区与浏览器不同**(如宿主跑在 UTC):北京时间 0-8 点的调用会记到前一日——「今日 ¥0」但月合计正常、余额对账报偏差,此前界面没有任何提示,极易误判为漏计。
- **改进**:设置页概览检测到宿主与浏览器时区不一致时显示双语提示(附宿主 IANA 时区名),说明日界差异与对齐方法(在宿主机以本地时区运行 dsh);不影响任何计费数值。

## v1.6.9(2026-08-28)—— 💰 计费审计:客户端/服务端口径漂移修复
- **修复:官方余额条「当日已用」漏计官方路由键**:侧边栏官方余额进度条的当日段此前只认 `deepseek` 前缀,而官方路由实际落账键为 `deepseek-official:模型`——当日已用恒为 0 或偏低,与对账口径不一致。现双前缀同计,并与服务端同口径剥离 `llm-` 包装前缀。
- **修复:会话徽章回退估算丢失峰谷档**:会话内含订阅(Plan)用量需拆分 API/Plan 金额、或旧快照缺精确成本时,客户端回退计价把峰谷价格子档剥掉——峰时调用按谷价(半价)重估。现峰/谷/历史子档随主档保留,峰/谷/周末/分界前四相位的档位与金额和服务端逐位一致。
- **修复:小数位设为 0 仍显示两位**(纯显示):客户端格式化把 decimals=0 误抬成 2,现与服务端同规则保留 0。
- **加固**:峰谷「生效时刻」为非法值时峰侧/谷侧判定归一;包装层重复清洗的保留键优先选上游真实键;官方渠道判定补 `llm-` 前缀剥离。
- 测试 verify.mjs 全量通过(新增 v1.6.9 审计回归块:双端四相位档位/金额一致性等行为级断言);完整审计报告见 docs/code-audit-2026-08-28.md。

## v1.6.8(2026-08-28)—— 🔐 密钥改由 DSH 凭据库托管 + 测试门禁
- **安全(P0):API Key 不再明文落盘**:此前账本文件把整个配置(含 OpenCode Go Key、各 Coding Plan Key、火山 AK/SK)原样写盘;现落盘前统一脱敏只留空占位,明文只存 DSH 凭据库,配置补丁入口同步加闸门防止密钥经更新设置回流。
- **安全(P0):密钥不再回传前端**:此前设置接口把配置原样下发给浏览器,密码框只是界面掩码;现下发的密钥字段恒为空,只附「已配置(来源)」状态;设置页输入框改为 write-only——保存后不回显,写入/清除走专用通道直达凭据库。
- **存量明文自动迁移(升级即迁移)**:升级后首次启动会自动把账本里已有的明文密钥逐项导入凭据库后清空;凭据库已有值不覆盖;凭据库不可写时**保留明文**并在设置页提示手动导出对应环境变量(绝不静默丢弃用户密钥),下个启动周期重试。
- **端点收紧**:自定义余额端点强制 https、禁止跟随重定向(防凭据跨源转发),凭据占位符发往白名单外主机时拒绝或警告;插件网络权限声明补全为代码实际出站的 16 个域名;额度类端点仅在显式启用对应 Provider 时出站。
- **工程化**:CI 在安装冒烟之外新增 verify.mjs 回归门禁与客户端产物漂移检查;`npm test` 可用;README 中英版补「密钥不落盘 / write-only 输入框」与隐私边界说明;补发 v1.6.4 / v1.6.5 / v1.6.6 release notes。
- **破坏性变更**:设置页密码框不再回显已存密钥(显示「已配置(来源)」状态);升级后首次启动自动迁移存量密钥,期间密钥解析短暂回落旧值,功能不中断。
- 测试 verify.mjs 全量通过(新增密钥治理回归块:落盘/下发双路径无明文、迁移五路径、strict codec 一致性、RPC 双侧对齐)。

## v1.6.7(2026-08-27)—— 💰 计费口径专项修复:峰谷生效时刻锚定 + 币种切换全量换基准
- **修复(高危):价格同步把「峰谷生效时刻」重置为「现在」**:官方价格页已无生效时间字段,同步的兜底分支把生效时刻写成同步时刻——此后任何历史重算(会话投影回放 / 按模型回填)中,同步时刻之前的**峰时事件全部被按谷价半价重算**。现官方页无值时不再改写该键;存量账本升级时自动钳制回 2026-08-16 16:00 UTC 历史分界(幂等迁移)。
- **修复:币种切换不重算历史 → 新旧口径并存(「误差更大」的直接来源)**:切到 CNY 后价格表换 ¥ 价而账本不动,会话徽章却是 CNY 口径——插件自身两个表面对同一天差 ~5%。现**切换币种即全量换基准**:后台回放会话日志按新价目逐事件重定价(日志覆盖完整的日子整体替换,已清理日志的会话保持原口径),历史与官方账单同基准,完成后有提示;存量混口径账本升级自动重算一次。
- **文档化已知近似**:客户端回退计价(会话徽章估算路径)按当前时刻档位定价,跨峰谷会话在峰时会高估谷时部分,精确金额以账本为准;#65 的 6 倍高估与 #62 修复前的历史形态吻合,升级即解。
- 测试 verify.mjs 全量通过(新增源哨兵 / 半价回归 / clamp 幂等 / CNY 重算端到端);真实账本影子验证:今日总额 ¥5.83 → ¥5.74,与官方 ¥5.64 偏差从 ~3.4% 收窄至 ~1.7%。

## v1.6.6(2026-08-27)—— 峰谷档位按请求发起时刻判定
- **改进:峰谷档位按请求发起时刻判定**:流式调用可能跨峰谷边界整点,以完成时刻归档会把数分钟前发起的请求算进另一个峰位;billing-stream 在请求发起时刻捕获时间戳传入账本,消除跨点调用的档位分歧。
- **改进:概览页新增口径脚注**:本地金额为最近一次落盘快照,与官方实时账单存在分钟级时差属预期;对账以金额为准。

## v1.6.5(2026-08-27)—— 官方余额对账双错修复
- **修复:今日官方费用恒 $0 + ¥ 变动错标 $**:官方渠道判定漏了 `deepseek-official` 前缀键;对账直接拿 ¥ 计价的余额差减 USD 计价的本地账本,跨币种错配几乎恒报 drift。现渠道白名单补入 `deepseek-official`,余额差先折算 USD 再比阈值,提示金额按余额真实币种取符号。

## v1.6.4(2026-08-27)—— 全新安装 ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION 修复(issue #72)
- **修复:全新安装被 pnpm「最小发布年龄」策略拒绝**:三个运行时依赖由浮动区间改为精确锁版——锁定版本发布时间固定不变,对任意年龄阈值永远满足,该错误对本插件不再可能复发。
