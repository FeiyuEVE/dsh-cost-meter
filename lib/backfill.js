...
/**
 * 一次性回填:把会话日志里尚未入库的会话消耗统计写入 sqlite 调用明细
 * (issue #78 后续,页面统计以 sqlite 为数据源)。与 importLegacyHistory 同源
 * 回放(逐事件计价、fork 种子剔除、包装层指纹去重、标题取日志最后一次
 * session/title),但目标直写 calls-db、不触碰账本。
 *
 * 去重(绝不与实时计费重复):
 *  - 按 (day_key, session_id) 对调用明细**现有全部行**(实时行与迁移行)判重:
 *    已存在的会话只补空标题(日志标题是权威来源),金额绝不动;
 *  - 同一会话跨安装时刻:安装后部分已有实时行 → 整会话跳过,安装前用量
 *    不计入(与 importLegacyHistory 同语义,无法安全拆分);
 *  - 脚本可重复执行(幂等),增量补齐后来新增的日志。
 * @param callsDb - 已打开的 CallsDb。
 * @param ledger - 已打开的账本(提供计价配置)。
 * @param sessionsRoot - 宿主会话根目录($DSH_HOME/sessions)。
 * @returns { scanned, imported, skipped, titled } 扫描日志数 / 新增会话日数 /
 *   已存在跳过数 / 补标题行数。
 */
export async function importCallsFromLogs(callsDb, ledger, sessionsRoot) {
  const result = { scanned: 0, imported: 0, skipped: 0, titled: 0 }
  const bySession = new Map()
  let scannedCount = 0
  ...
  // 汇总每个日期下有实际用量的会话(桶 calls>0 才参与),组装账本形状的日记录。
  ...
  for (const [date, list] of dateSessions) {
    ...
    const stats = callsDb.importLedgerDayMissing(target, ledger.config.exchangeRate)
    result.imported += stats.imported
    result.skipped += stats.skipped
    result.titled += stats.titled
  }
  return result
}
