...
import { OFFICIAL_PRICING_URL, OFFICIAL_PRICING_URL_ZH, LEGACY_BASE_BOUNDARY, normalizePrice, parsePricingHtml, costOf, providerPriceEntryFor, buildPriceCatalog, usdFromCost, resolveCurrency, isWrapperProviderId, wrapperUpstreamProvider, isPeakHour, DEFAULT_PEAK_EFFECTIVE_AT, DEFAULT_PEAK_WINDOWS } from './pricing.js'
import { createUsageDeduper, USAGE_DEDUP_WINDOW_MS, usageFingerprint } from './usage-dedup.js'
import { CallsDb, dayFromRows, dayRecord, sessionRows } from './calls-db.js'
...
function makeCostUsageProjection(ledger) {
  ...
  return {
    ...
    apply(state, event) {
      ...
      // 按生效计费币种折算为美元入账(issue #78):条目显式 currency → 峰谷
      // 上下文(prices.currency)→ 供应商表级 currency → USD。
      const priced = resolved.priced ? costOf(buckets, resolved.entry, atMs, peak) : 0
      const billed = usdFromCost(priced, resolveCurrency(resolved.entry, resolved.billingMode, ledger.config.prices, effectiveProvider), ledger.config.exchangeRate)
 ...
 * @param autoRoute - 自动模型路由运行状态(null = 不附加,兼容旧调用方)。
 */
async function buildState(ledger, callsDb, balance = emptyBalance(), goQuota = emptyGoQuota(), codingPlans = {}, customBalance = emptyCustomBalance(), reconcile = { ok: true, message: '' }, ctx = null, autoRoute = null) {
  const now = Date.now()
  ...
  // 页面整体统计以 sqlite 调用明细为数据源(issue #78):实时行 + 升级前迁移
  // 聚合行;成本统一人民币入账,下发 wire 按当前展示汇率折算回美元。
  // callsDb 缺席(异常降级/测试)时回退账本聚合,保持可用性。
  const rate = ledger.config.exchangeRate
  const sqliteToday = () => dayRecord(callsDb.rowsBetween(dayKey, dayKey), rate, dayKey)
  const sqliteRange = (fromKey, toKey, date) => dayFromRows(callsDb.rowsBetween(fromKey, toKey), rate, date, false)
  const sqliteAll = () => dayFromRows(callsDb.allRows(), rate, 'total', false)
  const sqliteTodayAgg = callsDb === undefined ? ledger.today() : sqliteToday()
  const sqliteMonthAgg = callsDb === undefined ? ledger.sumDays(monthKey) : sqliteRange(`${monthKey}-01`, `${monthKey}-31`, monthKey)
  const sqliteTotalAgg = callsDb === undefined ? ledger.sumDays(undefined) : sqliteAll()
  // 历史(90 天):按日聚合,不携带会话明细(明细由 getDaySessions 按需展开)。
  const sqliteHistory = () => {
    const from = localDayKey(now - 89 * 86_400_000)
    const rows = callsDb.rowsBetween(from, dayKey)
    const byDay = new Map()
    for (const row of rows) {
      let list = byDay.get(row.day_key)
      if (list === undefined) byDay.set(row.day_key, list = [])
      list.push(row)
    }
    return [...byDay.entries()]
      .sort((a, b) => (a[0] < b[0] ? 1 : -1))
      .map(([date, dayRows]) => dayFromRows(dayRows, rate, date, false))
  }
  const today = sqliteTodayAgg
  const month = sqliteMonthAgg
  const total = sqliteTotalAgg
  const history = callsDb === undefined ? ledger.history(90) : sqliteHistory()
  ...
  let budgetUsed
  if (budget.period === 'day') budgetUsed = budgetCostOf(today)
  else if (budget.period === 'all') budgetUsed = budgetCostOf(total)
    ...
    budgetUsed = start === null ? 0
      : budgetCostOf(callsDb === undefined
        ? ledger.sumRange(start, end)
        : sqliteRange(start, end, `${start}..${end}`))
  } else budgetUsed = budgetCostOf(month)
  ...
  const state = {
    ...
    // 自动模型路由运行状态(v1.7):缺席 = 旧宿主/未启用,客户端按配置自行兜底。
    ...(autoRoute !== null && typeof autoRoute === 'object' ? { autoRoute } : {}),
  }
...
/**
 * 对账用「本地今日官方渠道费用」(美元,与余额当日变动同基准比较)。
 * 统一从 sqlite 调用明细取数(issue #78):官方渠道成本人民币口径,
 * 按当前展示汇率折回美元——与页面统计同源,多 profile(web/acp)共用
 * 同一份明细;callsDb 缺席(异常降级/测试)回退账本聚合。
 * @param callsDb - 调用明细库(可 undefined)。
 * @param ledger - 账本(提供汇率配置与回退聚合)。
 * @param nowMs - 当前时刻。
 * @returns 今日官方渠道费用(美元,非负)。
 */
function reconcileTodayCost(callsDb, ledger, nowMs) {
  if (callsDb === undefined) return ledger.todayOfficialCost()
  const configured = Number(ledger.config.exchangeRate)
  const rate = Number.isFinite(configured) && configured > 0 ? configured : 7.2
  return callsDb.officialChannelCostRmb(localDayKey(nowMs)) / rate
}
...
function createService(ctx, ledger, callsDb) {
  ...
  // ── 自动模型路由(v1.7):平价/高峰时段自动切换正在运行的会话的 provider ──
  // 平价(谷)时段 → deepseek 官方路由;高峰时段 → OpenCode Go 路由;模型默认
  // deepseek-v4-flash + max 思考深度(两侧路由可配)。仅在当前会话模型为
  // deepseek-v4-flash 时执行——用户手动改选其它模型后不再干预(切换只改
  // provider,模型始终是 flash,因此「非 flash 即用户有意选择」恒成立)。
  // 会话身份由客户端经 setActiveSession 上报(composer dock 组件持有
  // sessionId),跨会话切换时服务端按最新上报的会话执行。
  let autoRouteStatus = { enabled: false, tier: null }
  let activeSessionId = ''
  let autoRouteBusy = false
  // 已应用标记:会话×档位,避免重复写模型选择事件;会话切换后自动失效。
  let autoRouteApplied = { sessionId: '', tier: null }
  let autoRouteTimer = null

  /** 自动路由配置(未配置键回退默认值)。 */
  const autoRouteConfig = () => ledger.config?.autoRoute ?? {}
  /** 当前档位:peak(高峰)| offpeak(平价,含周末全天)。 */
  const autoRouteTierOf = atMs => isPeakHour(atMs, Date.parse(DEFAULT_PEAK_EFFECTIVE_AT), DEFAULT_PEAK_WINDOWS) ? 'peak' : 'offpeak'
  /** 会话当前模型选择(会话级 pending → lastUsed;无则 null)。 */
  const autoRouteSessionModel = sessionId => {
    try {
      const session = ctx.get('sessions')?.get?.(sessionId)
      if (session === undefined || session === null) return null
      const state = ctx.get('sessionProjections')?.stateOf?.(session, 'modelSelection')
      if (state === null || state === undefined) return null
      const pending = state.pending
      const lastUsed = state.lastUsed
      if (pending !== null && pending !== undefined) return pending
      if (lastUsed !== null && lastUsed !== undefined) return lastUsed
      return null
    } catch (error) {
      console.warn(`[dsh-cost-meter] 读取会话模型选择失败: ${String(error?.message ?? error)}`)
      return null
    }
  }

  /** 立即评估一次自动路由(档位变化/会话变化/配置变化时调用;幂等)。 */
  const evaluateAutoRoute = async () => {
    const config = autoRouteConfig()
    const enabled = config.enabled === true
    const tier = autoRouteTierOf(Date.now())
    if (!enabled) {
      if (autoRouteStatus.enabled !== false || autoRouteStatus.tier !== null) {
        autoRouteStatus = { enabled: false, tier: null }
      }
      return
    }
    if (typeof activeSessionId !== 'string' || activeSessionId.length === 0) {
      autoRouteStatus = { enabled: true, tier, skipped: 'no-session' }
      return
    }
    // 只对「当前模型为 deepseek-v4-flash」的会话自动切换:读不到会话模型
    // (新会话尚无请求)按 flash 处理,允许路由首切。
    const current = autoRouteSessionModel(activeSessionId)
    if (current !== null && current !== undefined && current.model !== 'deepseek-v4-flash') {
      autoRouteStatus = { enabled: true, tier, skipped: 'non-flash', sessionId: activeSessionId }
      return
    }
    if (autoRouteApplied.sessionId === activeSessionId && autoRouteApplied.tier === tier) {
      autoRouteStatus = {
        enabled: true, tier, sessionId: activeSessionId,
        provider: config[tier]?.provider, model: config[tier]?.model,
        reasoningEffort: config[tier]?.reasoningEffort,
        ...(autoRouteStatus.lastSwitchAt === undefined ? {} : { lastSwitchAt: autoRouteStatus.lastSwitchAt }),
      }
      return
    }
    if (autoRouteBusy) return
    autoRouteBusy = true
    try {
      const controller = ctx.get('sessionController')
      if (controller === undefined || controller === null || typeof controller.selectModel !== 'function') {
        autoRouteStatus = { enabled: true, tier, sessionId: activeSessionId, skipped: 'no-controller' }
        return
      }
      ...
      await controller.selectModel({
        ...
        model: route.model,
        ...(typeof route.reasoningEffort === 'string' && route.reasoningEffort.length > 0
          ? { reasoningEffort: route.reasoningEffort }
          : {}),
      })
      autoRouteApplied = { sessionId: activeSessionId, tier }
      autoRouteStatus = {
        enabled: true,
        tier,
        provider: route.provider,
      ...
      console.log(`[dsh-cost-meter] 自动模型路由:${tier === 'peak' ? '高峰' : '平价'} → ${String(route.provider)}/${String(route.model)}(${String(route.reasoningEffort ?? '')})(会话 ${activeSessionId})`)
    } catch (error) {
      console.warn(`[dsh-cost-meter] 自动模型路由切换失败: ${String(error?.message ?? error)}`)
      autoRouteStatus = { enabled: true, tier, sessionId: activeSessionId, error: String(error?.message ?? error) }
    } finally {
      autoRouteBusy = false
    }
  }

  /** (重)启自动路由轮询:按配置间隔建定时器,并立即评估一次。 */
  const restartAutoRouteTimer = () => {
    if (autoRouteTimer !== null) { clearInterval(autoRouteTimer); autoRouteTimer = null }
    const config = autoRouteConfig()
    const enabled = config.enabled === true
    if (!enabled) {
      autoRouteStatus = { enabled: false, tier: null }
    ...
    const minutes = Number.isInteger(config.checkIntervalMinutes) && config.checkIntervalMinutes >= 1 && config.checkIntervalMinutes <= 60
      ? config.checkIntervalMinutes
      : 1
    autoRouteTimer = setInterval(() => { void evaluateAutoRoute() }, minutes * 60_000)
    autoRouteTimer.unref?.()
    void evaluateAutoRoute()
  }
  restartAutoRouteTimer()
  ctx.effect(() => () => {
    if (autoRouteTimer !== null) clearInterval(autoRouteTimer)
    autoRouteTimer = null
  }, 'cost-meter: auto route timer')
  ...
  const ensureBalance = async (force = false) => {
    ...
    const task = queryBalance(ctx, localeOf(ledger.config)).then(result => {
      ...
      if ((ledger.config?.balance?.reconcile ?? true) === true && balanceCache.value.status === 'ok') {
        ...
        // 本地账本今日费用按余额条目币种展示(CNY 账号按对账所用同一汇率折算
        // 为 ¥),与余额当日变动同币种直接可比——此前本地一侧恒为 $ 口径
        // (issue #78 统一人民币展示)。
        const localCost = event => {
          const isCny = String(event.spentCurrency ?? '').toUpperCase() === 'CNY'
          if (!isCny) return usd(event.todayCost)
          const configured = Number(ledger.config.exchangeRate)
          const rate = Number.isFinite(configured) && configured > 0 ? configured : 7.2
          return '¥' + Number(event.todayCost * rate).toFixed(4)
        }
        // 变动金额按余额条目的真实币种显示符号(CNY 余额账号此前被错标成 $),
        // 并附 ≈USD 折算参考(zh/en 两份模板共用参数,不做字典改动)。
        ...
        const { ref, event } = reconcileBalanceDelta(ledger.balanceRef, balanceCache.value, reconcileTodayCost(callsDb, ledger, nowMs), localDayKey(nowMs), nowMs, { exchangeRate: ledger.config.exchangeRate })
        if (ref !== ledger.balanceRef) {
          ledger.balanceRef = ref
          ledger.scheduleWrite()
        }
        reconcileNotice = event !== null && event.kind === 'drift'
          ? { ok: false, message: tmsg(localeOf(ledger.config), 'reconcileWarn', { cost: localCost(event), delta: nativeDelta(event) }) }
          : { ok: true, message: '' }
      }
    }, error => {
  ...
  const build = async (forceBalance = false) => {
    ...
    return await buildState(ledger, callsDb, balanceCache.value, goQuotaCache.value, await mergedCodingPlans(), customBalanceCache.value, reconcileNotice, ctx, autoRouteStatus)
  }
  ...
  const service = {
    ...
    async updateConfig(patch) {
      ...
      // 自动路由配置变更(v1.7):立即按新配置重启轮询并评估一次(开/关/改路由/
      // 改间隔都走这里;已应用标记在开关关闭后失效,重开即重评估)。
      if (patch !== null && typeof patch === 'object' && patch.autoRoute !== undefined) {
        restartAutoRouteTimer()
      }
      ...
      // sqlite 调用明细与账本同源同步(issue #78):仅当账本历史本身被重算时
      // 才动明细——本地来源归零(__local__/本地推理)、Plan 分类重拆、币种
      // 切换全量重定价;迁移行按重算后的账本重建,实时行就地重算。
      try {
        if (patch !== null && typeof patch === 'object' && patch.priceOverrides !== undefined) {
          callsDb.zeroLocalCosts(ledger.config)
          callsDb.rebuildAggregates(ledger.days, ledger.config.exchangeRate)
        }
        if (patch !== null && typeof patch === 'object'
          && (patch.planBilling !== undefined || patch.codingPlans !== undefined || patch.goQuota !== undefined)) {
          callsDb.repriceAll(ledger.config)
          callsDb.rebuildAggregates(ledger.days, ledger.config.exchangeRate)
        }
        if (currencyBefore !== currencyAfter) {
          callsDb.repriceAll(ledger.config)
        ...
        console.warn(`[dsh-cost-meter] 调用明细随账本重算失败: ${String(error?.message ?? error)}`)
      }
      ledger.scheduleWrite()
      return build(false)
    },
    ...
    async refreshBalance() {
      ...
      return {
        ...
        state: await buildState(ledger, callsDb, value, goQuotaCache.value, await mergedCodingPlans(), customBalanceCache.value, reconcileNotice, ctx, autoRouteStatus),
      }
    },
    ...
    async refreshCustomBalance() {
      ...
      return {
        ...
        state: await buildState(ledger, callsDb, balanceCache.value, goQuotaCache.value, await mergedCodingPlans(), value, reconcileNotice, ctx, autoRouteStatus),
      }
    },
    ...
    async refreshGoQuota() {
      ...
      return {
        ...
        state: await buildState(ledger, callsDb, balanceCache.value, value, await mergedCodingPlans(), customBalanceCache.value, reconcileNotice, ctx, autoRouteStatus),
      }
    },
    ...
    async refreshCodingPlan(provider) {
      ...
      return {
        ...
        state: await buildState(ledger, callsDb, balanceCache.value, goQuotaCache.value, await mergedCodingPlans(), customBalanceCache.value, reconcileNotice, ctx, autoRouteStatus),
      }
    },
    ...
    async fetchPrices() {
      ...
      try {
        ...
        // 币种翻转后 sqlite 调用明细随账本重定价(实时行就地重算,迁移行重建)。
        if (currencyBefore !== currencyAfter) {
          try {
            callsDb.repriceAll(ledger.config)
            callsDb.rebuildAggregates(ledger.days, ledger.config.exchangeRate)
          } catch (error) {
            console.warn(`[dsh-cost-meter] 调用明细随官方同步重算失败: ${String(error?.message ?? error)}`)
          }
        }
    ...
    async resetHistory() {
      ...
      // 与「清空全部历史」语义不一致。sqlite 调用明细同步清空(页面统计数据源)。
      ledger.days = {}
      ledger.planSamples = {}
      ledger.planHourBuckets = {}
    ...
    // 上报当前正在查看/运行的会话(v1.7,自动模型路由用):只在档位切换时对
    // 最新上报的会话执行 provider 切换;空串 = 离开会话,清除已应用标记。
    async setActiveSession(sessionId) {
      const locale = localeOf(ledger.config)
      const next = typeof sessionId === 'string' ? sessionId.trim() : ''
      if (next === '') {
        activeSessionId = ''
        autoRouteApplied = { sessionId: '', tier: null }
        return { ok: true, message: '', state: await build(false) }
      }
      if (next !== activeSessionId) {
        activeSessionId = next
        // 会话变化:旧会话的应用标记作废,立即按新会话模型评估一次。
        autoRouteApplied = { sessionId: '', tier: null }
        void evaluateAutoRoute()
      }
      return { ok: true, message: '', state: await build(false) }
    ...
    async importLegacyHistory() {
      ...
      try {
        ...
        // 导入结果同步进 sqlite 迁移行(去重感知:与实时行重叠的会话只补标题)。
        for (const day of Object.values(ledger.days)) callsDb.importLedgerDayMissing(day, ledger.config.exchangeRate)
    ...
    async getDaySessions(date) {
      ...
      // sqlite 调用明细为准;缺失日返回零值记录。
      if (callsDb !== undefined) return dayRecord(callsDb.rowsBetween(date, date), ledger.config.exchangeRate, date)
    ...
    async getTopSessions(limit, sort = 'cost', dir = 'desc') {
      ...
      const asc = dir === 'asc'
      // sqlite 调用明细为准:跨全部日期按 (day_key, session_id) 分组;
      // 旧路径(无 sqlite)回退账本按日会话遍历。
      const all = callsDb !== undefined
        ? sessionRows(callsDb.allRows(), ledger.config.exchangeRate)
        : (() => {
          const rows = []
          ...
          for (const date of dateKeys) {
            ...
            const dayRows = day.sessions.slice()
            if (sortKey === 'recent' && !asc) dayRows.reverse()
            for (const s of dayRows) {
              if (s === null || typeof s !== 'object') continue
              const row = { date, id: String(s.id ?? ''), input: s.input ?? 0, output: s.output ?? 0, cacheRead: s.cacheRead ?? 0, cacheWrite: s.cacheWrite ?? 0, reasoning: s.reasoning ?? 0, calls: s.calls ?? 0, cost: s.cost ?? 0, apiCost: s.apiCost ?? s.cost ?? 0, byProviderModel: s.byProviderModel ?? {} }
      ...
      } else if (sortKey === 'recent' && callsDb !== undefined) {
        // sqlite 路径无构造序:recent 复刻账本语义 = 日倒序 + 每日首记倒序
        // (firstId 为会话在该日最早一行的自增 id,即账本会话数组的入列顺序)。
        all.sort((a, b) => {
          const dc = asc ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date)
          if (dc !== 0) return dc
          return asc ? a.firstId - b.firstId : b.firstId - a.firstId
        })
      }
      // firstId 是排序内部字段,不下发 wire。
      for (const row of all) delete row.firstId
      // recent 旧路径已按构造顺序排好,不再重排。
...
export function apply(ctx) {
  ...
  // 打开调用明细库(页面统计唯一数据源,issue #78);升级前账本历史以迁移行
  // 幂等导入(去重感知:与实时行重叠的会话只补标题,不重复计费)。
  const callsDb = CallsDb.open()
  const syncLedgerToCallsDb = () => {
    try {
      for (const day of Object.values(ledger.days)) callsDb.importLedgerDayMissing(day, ledger.config.exchangeRate)
    } catch (error) {
      console.warn(`[dsh-cost-meter] 账本历史导入调用明细库失败: ${String(error?.message ?? error)}`)
    }
  }
  syncLedgerToCallsDb()
  console.log(`[dsh-cost-meter] 已加载,账本:${ledger.path},调用明细:${callsDb.path}`)

  // 卸载/退出前最终落盘。
  ctx.effect(() => () => { ledger.close(); callsDb.close() }, 'cost-meter: ledger & calls-db close')

  // 会话元信息(工作区/标题):session/created 提供 cwd,session/title 事件提供
  // 标题;两者都是会话作用域事件,根插件监听须 global:true(tools/invariant 同款)。
  // llm/stream 计费回调发生时据此补全调用明细的工作区与会话标题字段。
  // 恢复/回放会话的 session/title 事件在种子段内不会重发到 firehose,
  // 创建时直接从会话日志取最后一次标题,保证恢复会话的实时调用行也带标题。
  ...
  ctx.on('session/created', (session) => {
    ...
    for (let i = events.length - 1; i >= 0; i -= 1) {
      const ev = events[i]
      if (ev !== null && typeof ev === 'object' && ev.type === 'session/title'
        && typeof ev.data?.title === 'string' && ev.data.title.length > 0) {
        title = ev.data.title
        break
      }
    }
    const prev = sessionMeta.get(session.id) ?? {}
  ...
  ctx.on('session/event', (session, event) => {
    if (session === null || typeof session !== 'object' || typeof session.id !== 'string') return
    if (event === null || typeof event !== 'object' || event.type !== 'session/title') return
    const title = event.data?.title
    if (typeof title !== 'string' || title.length === 0) return
    const prev = sessionMeta.get(session.id) ?? {}
    sessionMeta.set(session.id, { ...prev, title })
  }, { global: true })
  ctx.effect(() => () => sessionMeta.clear(), 'cost-meter: session meta')
  ...
  const backfillTimer = setTimeout(async () => {
    ...
    runStartupImports(ledger, join(resolveDshHome(), 'sessions')).then(() => {
      syncLedgerToCallsDb()
    }).catch(error => {
  ...
  ctx.on('llm/stream', createLlmStreamBilling({
    account: (usage, model, sessionId, atMs, provider) => {
      ...
      // 账本入账并返回定价记录;同值写入 sqlite 调用明细(工作区/会话标题
      // 来自 sessionMeta 追踪;成本统一人民币口径)。
      const priced = ledger.account(buckets, model, sessionId, atMs, effective)
      if (priced === undefined) return
      const meta = typeof sessionId === 'string' ? (sessionMeta.get(sessionId) ?? {}) : {}
      try {
        callsDb.record({
          workspace: meta.cwd ?? '',
          ...
          costRmb: priced.costRmb,
          apiCostRmb: priced.apiCostRmb,
        })
      } catch (error) {
        // 账本已入账而明细写库失败(磁盘满/权限/并发锁超时等):响亮告警,
        // 不打断模型流;下次启动账本迁移会以聚合行补平该日统计。
        console.warn(`[dsh-cost-meter] 调用明细写 SQLite 失败(账本已入账): ${String(error?.message ?? error)}`)
      }
    },
  }))
  ...
  ctx.provide('costMeter', createService(ctx, ledger, callsDb))
}
