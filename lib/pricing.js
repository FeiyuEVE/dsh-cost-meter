...
export function normalizePrice(value) {
  ...
  // 条目级货币配置(issue #78):USD | CNY;未配置 = 跟随解析上下文
  // (deepseek-peak 主表 → prices.currency,第三方 → USD)。
  const currency = value.currency === 'CNY' ? 'CNY' : value.currency === 'USD' ? 'USD' : null
...
/**
 * 解析一次调用的**生效计费币种**(issue #78)。
 * 优先级:条目显式 currency → deepseek-peak 上下文(prices.currency)
 * → 供应商表级 currency(prices.providers.<id>.currency)→ USD。
 * @param entry - 命中的价格记录(可 null)。
 * @param billingMode - 'deepseek-peak' | 'flat' | 'batch'。
 * @param prices - 价表(含顶层 currency 与 providers.<id>.currency)。
 * @param provider - 请求 provider id(归一化小写;缺省跳过供应商级配置)。
 * @returns 'USD' | 'CNY'。
 */
export function resolveCurrency(entry, billingMode, prices, provider) {
  if (entry !== null && typeof entry === 'object' && (entry.currency === 'CNY' || entry.currency === 'USD')) {
    return entry.currency
  }
  if (billingMode === 'deepseek-peak') return prices?.currency === 'CNY' ? 'CNY' : 'USD'
  if (typeof provider === 'string' && provider.length > 0) {
    // 与 providerPriceEntryFor 同口径归一(llm- 前缀剥离 + 小写),确保表级
    // 币种按与价格解析一致的渠道键命中。
    let id = provider.trim().toLowerCase()
    if (id.startsWith('llm-')) id = id.slice(4)
    const table = prices?.providers?.[id]
    if (table !== null && typeof table === 'object' && (table.currency === 'CNY' || table.currency === 'USD')) {
...
/**
 * 把按「生效计费币种」计算的成本统一折算为人民币(issue #78)。
 * 人民币价表(CNY)原值;美元价表(USD)按展示汇率乘算。汇率非法(<=0 /
 * 非有限数)按 1 兜底,避免 NaN 污染。sqlite 调用明细的成本一律以此入账,
 * 页面统计以人民币为统一口径。
 * @param cost - 按生效币种计出的成本。
 * @param currency - 生效币种 'USD' | 'CNY'。
 * @param exchangeRate - 展示汇率(美元 → 人民币)。
 * @returns 人民币成本(非负)。
 */
export function rmbFromCost(cost, currency, exchangeRate) {
  const value = Math.max(0, Number(cost) || 0)
  if (currency === 'CNY') return value
  const rate = Number(exchangeRate)
  return value * (Number.isFinite(rate) && rate > 0 ? rate : 1)
