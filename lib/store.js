...
export function defaultConfig() {
  return {
    ...
    // 自动模型路由:平价(谷)时段自动切 DeepSeek 官方,高峰时段自动切 OpenCode Go;
    // 两条路由默认 deepseek-v4-flash + max 思考深度(可配)。仅对当前会话模型为
    // deepseek-v4-flash 的会话生效——用户手动改选其它模型后不再干预。
    autoRoute: {
      enabled: false, // 总开关(默认关,设置页开启)
      checkIntervalMinutes: 1, // 档位轮询间隔(分钟,1-60)
      offPeak: { provider: 'deepseek-official', model: 'deepseek-v4-flash', reasoningEffort: 'max' },
      peak: { provider: 'opencode-go', model: 'deepseek-v4-flash', reasoningEffort: 'max' },
    },
    planBilling: {
...
const VALIDATION_MESSAGES = {
  zh: {
    ...
    autoRoute: 'autoRoute 非法',
    autoRouteEnabled: 'autoRoute.enabled 必须是布尔值',
    autoRouteInterval: 'autoRoute.checkIntervalMinutes 必须是 1-60 的整数',
    autoRouteSide: 'autoRoute.{side} 必须含非空 provider 与 model 字符串',
    autoRouteEffort: 'autoRoute.{side}.reasoningEffort 必须是 off / low / high / max',
  ...
  en: {
    ...
    autoRoute: 'Invalid autoRoute',
    autoRouteEnabled: 'autoRoute.enabled must be a boolean',
    autoRouteInterval: 'autoRoute.checkIntervalMinutes must be an integer from 1 to 60',
    autoRouteSide: 'autoRoute.{side} must have non-empty provider and model strings',
    autoRouteEffort: 'autoRoute.{side}.reasoningEffort must be off / low / high / max',
...
export function applyConfigPatch(current, rawPatch) {
  ...
  // 自动模型路由:整体结构与双侧路由合法才接受;对合法字段做就近清洗
  // (间隔夹取 1-60,provider/model 去空白)。
  const autoRoute = candidate.autoRoute
  if (autoRoute === null || typeof autoRoute !== 'object' || Array.isArray(autoRoute)) {
    errors.push(vmsg(locale, 'autoRoute'))
  } else {
    if (typeof autoRoute.enabled !== 'boolean') errors.push(vmsg(locale, 'autoRouteEnabled'))
    const autoRouteInterval = Number(autoRoute.checkIntervalMinutes)
    if (!Number.isInteger(autoRouteInterval) || autoRouteInterval < 1 || autoRouteInterval > 60) {
      errors.push(vmsg(locale, 'autoRouteInterval'))
    } else {
      autoRoute.checkIntervalMinutes = autoRouteInterval
    }
    for (const side of ['offPeak', 'peak']) {
      const route = autoRoute[side]
      if (route === null || typeof route !== 'object' || Array.isArray(route)
        || typeof route.provider !== 'string' || route.provider.trim().length === 0
        || typeof route.model !== 'string' || route.model.trim().length === 0) {
        errors.push(vmsg(locale, 'autoRouteSide', { side }))
        continue
      }
      route.provider = route.provider.trim()
      route.model = route.model.trim()
      if (route.reasoningEffort !== undefined
        && !['off', 'low', 'high', 'max'].includes(route.reasoningEffort)) {
        errors.push(vmsg(locale, 'autoRouteEffort', { side }))
      }
    }
  }
    ...
    if (prices.providers !== undefined) {
      if (prices.providers === null || typeof prices.providers !== 'object' || Array.isArray(prices.providers)) {
        ...
        for (const [provider, providerTable] of Object.entries(prices.providers)) {
          ...
          // 供应商表级币种(issue #78):非法值拒绝,合法值作为该家模型的缺省币种。
          if (providerTable.currency !== undefined && providerTable.currency !== 'USD' && providerTable.currency !== 'CNY') {
            errors.push(vmsg(locale, 'pricesCurrency'))
            continue
          }
...
export function sanitizeConfig(raw) {
  ...
  // 供应商级/模型级币种配置(issue #78):表级非法值收敛 USD;模型级非法值
  // 剥离(与 normalizePrice 同语义,回落到表级/上下文默认,不误标币种)。
  if (out.prices !== null && typeof out.prices === 'object' && !Array.isArray(out.prices)
    && out.prices.providers !== null && typeof out.prices.providers === 'object' && !Array.isArray(out.prices.providers)) {
    for (const table of Object.values(out.prices.providers)) {
      if (table === null || typeof table !== 'object' || Array.isArray(table)) continue
      if (table.currency !== undefined) table.currency = oneOf(table.currency, ['USD', 'CNY'], 'USD')
      if (table.models === null || typeof table.models !== 'object' || Array.isArray(table.models)) continue
      for (const entry of Object.values(table.models)) {
        if (entry !== null && typeof entry === 'object' && !Array.isArray(entry) && entry.currency !== undefined
...
export class Ledger {
  ...
  account(tokens, modelId, sessionId, atMs, provider) {
    ...
    // 生效计费币种(issue #78):条目显式 currency → deepseek-peak 上下文
    // (prices.currency)→ 供应商表级 currency → USD。账本恒以美元存储;
    // sqlite 调用明细以人民币入账(rmbFromCost),页面统计统一人民币口径。
    const currency = resolveCurrency(resolved.entry, resolved.billingMode, this.config.prices, provider)
    const priced = resolved.priced ? costOf(tokens, entry, atMs, peak) : 0
    const cost = usdFromCost(priced, currency, this.config.exchangeRate)
    ...
    const costRmb = rmbFromCost(priced, currency, this.config.exchangeRate)
    const apiCostRmb = cls === 'api' ? costRmb : 0
    ...
    this.prune()
    this.scheduleWrite()
    // 返回本次入账的定价记录(sqlite 调用明细与账本同源同值;调用方不消费可忽略)。
    return {
      cost,
      apiCost,
      costRmb,
      ...
      dayKey: date,
      atMs,
      provider: typeof provider === 'string' && provider.length > 0 ? provider : 'deepseek',
      model: String(modelId ?? 'default'),
      buckets,
    }
  }
