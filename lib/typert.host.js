...
/** 自动模型路由单侧配置:provider 路由 + 模型 id + 可选思考深度。 */
const autoRouteRouteSchema = z.object({
  provider: z.string(),
  model: z.string(),
  reasoningEffort: z.enum(['off', 'low', 'high', 'max']).optional(),
})

/** 自动模型路由运行状态(服务下发):当前档位、已应用路由与最近一次切换/错误。 */
const autoRouteStatusSchema = z.object({
  enabled: z.boolean(),
  ...
  lastSwitchAt: num.optional(),
  // 跳过原因(当前会话非 flash / 等待会话 / 服务不可用等),无错误/跳过后缺席。
...
const providerPriceSchema = z.object({
  ...
  output: num.optional(),
  reasoning: num.optional(),
  // 条目级计费币种(issue #78):USD | CNY;缺席 = 供应商表级/上下文默认。
...
const priceSchema = z.object({
  ...
  output: num,
  reasoning: num.optional(),
  // 条目级计费币种(issue #78);DeepSeek 主表通常缺席,跟随 prices.currency。
  currency: z.enum(['USD', 'CNY']).optional(),
  billingMode: z.enum(['flat', 'deepseek-peak', 'batch']).optional(),
  offPeak: priceTierSchema.optional(),
  peak: priceTierSchema.optional(),
...
const configSchema = z.object({
  ...
  prices: z.object({
    ...
    default: priceSchema,
    // 供应商表级币种(issue #78):该家模型的缺省计费币种。
    providers: z.record(z.string(), z.object({ models: z.record(z.string(), providerPriceSchema), currency: z.enum(['USD', 'CNY']).optional() })).optional(),
  }),
  budget: z.object({
    enabled: z.boolean(),
    amount: num,
  ...
  // 自动模型路由(v1.7):平价/高峰自动切换 provider;模型默认 deepseek-v4-flash + max。
  autoRoute: z.object({
    enabled: z.boolean(),
    checkIntervalMinutes: num,
    offPeak: autoRouteRouteSchema,
    peak: autoRouteRouteSchema,
  }).optional(),
...
export const stateSchema = z.object({
  ...
  // 自动模型路由运行状态(v1.7):缺席 = 旧服务端 / 未启用时的空状态。
  autoRoute: autoRouteStatusSchema.optional(),
  meta: z.object({
    now: num,
...
const _sessionId$codec = { mode: 'strict', typeSymbol: 'dsh-cost-meter#SessionId', schema: z.string() }

export const TYPERT = {
  package: '@feiyueve/dsh-cost-meter',
  face: 'host',
  schemas: [],
  invocations: [
    {
    ...
    {
      // 自动模型路由(v1.7):上报当前正在查看/运行的会话 id,
      // 宿主在峰谷档位切换时只对该会话执行 provider 切换。
      id: 'dsh-cost-meter#costMeter/setActiveSession',
      service: 'costMeter',
      namespace: 'costMeter',
    ...
    services: [
      {
        ...
        members: [
          ...
          },
          {
            ...
            signature: 'setActiveSession(sessionId: SessionId): Promise<FetchPricesResult>',
            summary: '上报当前正在查看/运行的会话 id,供自动模型路由切换。Report the active session id for auto model routing.',
            jsDoc: '/**\n * 上报当前正在查看/运行的会话 id(v1.7):自动模型路由在峰谷档位切换时\n * 只对最近上报的会话执行 provider 切换;页面离开会话时传空串清除。\n * @param sessionId - 会话 id(空串 = 离开会话)。\n * @returns 查询结果与最新快照\n * Report the active session id (v1.7): auto model routing only switches the\n * most recently reported session on tier changes; pass an empty string when\n * leaving the session.\n * @param sessionId - Session id (empty string = no active session).\n * @returns The result and the latest snapshot\n */',
          },
        ],
        types: [],
      },
